...
# Compare against the main branch, because most development is done against it.
base_commit="$(git merge-base HEAD main)"
if [ "$base_commit" = "$(git rev-parse HEAD)" ]; then
  # Prefix of main branch, so compare against parent commit
  base_commit="$(git rev-parse HEAD^)"
  echo "Running clang-format against parent commit $base_commit"
else
  echo "Running clang-format against parent commit $base_commit from main branch"
fi
