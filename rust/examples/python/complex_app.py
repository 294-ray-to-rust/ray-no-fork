...
print("  Actors:        4 (DataStore, ProgressMonitor, Reducer, FraudDetector)")
print(f"  Task calls:    {total_tasks} (per-worker: {ray._runtime._task_call_counts})")
print("  ray.put():     7 calls")
print("  ray.get():     30+ calls")
print("  ray.wait():    3 calls (stats, validation, batch objects)")
