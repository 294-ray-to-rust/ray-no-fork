...
def test_resources():
    ...
    for fn, inp, expected in cases:
        ...
        assert (
            result == expected
        ), f"{fn.__name__}({inp}): expected {expected}, got {result}"
...
def test_actor_sync():
    ...
    signal = SignalActor.options(name="Signal_sync", namespace="test_sync").remote()
...
def test_pattern_async_actor():
    ...
    def _make_driver():
        return PyCoreWorker(
            1, "127.0.0.1", cluster.gcs_address(), 1, node_id=cluster.node_id()
        )
    ...
    assert (
        async_state["max_concurrent"] >= 2
    ), f"Async: expected max_concurrent>=2, got {async_state['max_concurrent']}"

    return (
        f"pattern_async_actor: sync max_concurrent={sync_state['max_concurrent']}, "
...
def test_actor_restart():
    ...
    ray._runtime.driver.kill_actor(bytes(last_actor._actor_id.binary()), True, True)
    error_on_dead = False
    try:
    ...
    return (
        f"actor_restart: {successful_calls} successful calls across "
...
def test_pattern_generators():
    ...
    return (
        f"pattern_generators: regular + generator multi-return "
...
def test_generator():
    ...
    return (
        f"generator: split({array_size}/{chunk_size})={num_chunks} chunks, "
...
def test_streaming_generator():
    ...
    return (
        "streaming_generator: iteration(5), error mid-stream, " "actor gen(5), wait(4)"
    )
...
def test_ray_oom_prevention():
    ...
    return (
        f"ray_oom_prevention: OOM task retried {oom_calls['n']}x, "
...
def test_namespaces():
    ...
    orange_c = NamedActor.options(name="orange", namespace="colors").remote(
        "orange_colors"
    )
    _purple_c = NamedActor.options(name="purple", namespace="colors").remote(
        "purple_colors"
    )
    ...
    orange_f = NamedActor.options(name="orange", namespace="fruits").remote(
        "orange_fruits"
    )
    _watermelon_f = NamedActor.options(name="watermelon", namespace="fruits").remote(
        "watermelon_fruits"
    )
    ...
    return (
        "namespaces: 2 actors in 'colors', 2 in 'fruits', "
...
def test_out_of_band_object_ref_serialization():
    ...
    return (
        "out_of_band_object_ref: put → pickle OID → free → object gone; "
...
TESTS = [
    ...
    (
        "getting_started.py (Counter)",
        "getting_started.py",
        test_getting_started_counter,
    ),
    ("actor-repr.py", "actor-repr.py", test_actor_repr),
    (
        "anti_pattern_global_variables.py",
        "anti_pattern_global_variables.py",
        test_global_var_actor,
    ),
    (
        "actor_checkpointing.py (Worker)",
        "actor_checkpointing.py",
        test_actor_checkpointing,
    ),
    (
        "actor_checkpointing.py (Immortal)",
        "actor_checkpointing.py",
        test_immortal_actor_checkpoint,
    ),
    (
        "monte_carlo_pi.py (Progress)",
        "monte_carlo_pi.py",
        test_monte_carlo_progress_actor,
    ),
    (
        "monte_carlo_pi.py (Pi calc)",
        "monte_carlo_pi.py",
        test_monte_carlo_pi_computation,
    ),
    (
        "pattern_tree_of_actors.py",
        "pattern_tree_of_actors.py",
        test_tree_of_actors_trainers,
    ),
    ...
    (
        "actor-http-server.py (Counter)",
        "actor-http-server.py",
        test_actor_http_server_counter,
    ),
    # ── New: actor return values ──
    (
        "actor return values (ray.get)",
        "(new: actor return values)",
        test_actor_return_values,
    ),
    # ── New: non-actor remote tasks ──
    ("getting_started.py (tasks)", "getting_started.py", test_getting_started_tasks),
    (
        "anti_pattern_closure_capture",
        "anti_pattern_closure_capture_large_objects.py",
        test_closure_capture_large_objects,
    ),
    (
        "anti_pattern_pass_large_arg",
        "anti_pattern_pass_large_arg_by_value.py",
        test_pass_large_arg_by_value,
    ),
    ("anti_pattern_ray_get_loop.py", "anti_pattern_ray_get_loop.py", test_ray_get_loop),
    (
        "anti_pattern_unnecessary_ray_get",
        "anti_pattern_unnecessary_ray_get.py",
        test_unnecessary_ray_get,
    ),
    (
        "anti_pattern_redefine_task_loop",
        "anti_pattern_redefine_task_actor_loop.py",
        test_redefine_task_loop,
    ),
    (
        "anti_pattern_too_fine_grained",
        "anti_pattern_too_fine_grained_tasks.py",
        test_too_fine_grained_tasks,
    ),
    (
        "anti_pattern_ray_get_submission",
        "anti_pattern_ray_get_submission_order.py",
        test_ray_get_submission_order,
    ),
    (
        "anti_pattern_ray_get_too_many",
        "anti_pattern_ray_get_too_many_objects.py",
        test_ray_get_too_many_objects,
    ),
    # ── New: object store ──
    ...
    (
        "anti_pattern_fork_new_processes",
        "anti_pattern_fork_new_processes.py",
        test_fork_new_processes,
    ),
    ...
    (
        ...
        "placement_group_capture_child_tasks_example.py",
        test_placement_group_capture_child_tasks,
    ),
    (
        "original_resource_unavailable_example.py",
        "original_resource_unavailable_example.py",
        test_original_resource_unavailable,
    ),
    ...
    (
        "anti_pattern_oob_obj_ref_ser.py",
        "anti_pattern_out_of_band_object_ref_serialization.py",
        test_out_of_band_object_ref_serialization,
    ),
]
