...
if p0_seqs == expected_seqs:
    print(
        f"  PASS  Single producer->consumer: {len(p0_items)} items, all sequences correct"
    )
    passed += 1
else:
...
for pid in ["P0", "P1", "P2"]:
    ...
    if seqs != list(range(items_per_producer)):
        all_correct = False
        print(
            f"  FAIL  Producer {pid}: expected {items_per_producer} items, got {len(seqs)}"
        )

if all_correct and len(collected) == 3 * items_per_producer:
    print(
        f"  PASS  3 producers -> consumer: {len(collected)} total items, all sequences correct"
    )
    passed += 1
...
if (
    stats["produced"] == items_to_produce
    and stats["consumed"] == items_to_produce
    and stats["buffered"] == 0
):
    print(
        f"  PASS  Bounded buffer: {stats['produced']} produced, {stats['consumed']} consumed, buffer empty"
    )
    passed += 1
else:
...
if total_taken == 100 and actual_processed == expected_processed:
    print(
        f"  PASS  Work-stealing: 100 items across {len(stats)} consumers, distribution={stats}"
    )
    passed += 1
else:
